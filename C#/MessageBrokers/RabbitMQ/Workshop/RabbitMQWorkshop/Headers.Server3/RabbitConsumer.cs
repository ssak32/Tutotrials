...
namespace Header.Server3
{
    public class RabbitConsumer : IDisposable
    {
        const string _queueName = "WSQueue.Headers3";
        ...
        private void Poll()
        {
            while (Enabled)
            {
                ...
                foreach (var headerKey in delivaryArgs.BasicProperties.Headers.Keys)
                {
                    var headerValue = delivaryArgs.BasicProperties.Headers[headerKey];
